// const image = require('../../../ecommercegrupo05/src/assets/coder2.png')



let registerTemplate =  `<html lang="en">
    <head>
        <meta charset='utf-8' />
        <meta name='viewport' content='width=device-width, initial-scale=1' />
        <title>Unknow Code</title>
    </head>
    <body style=" font-family: 'Open Sans', 'Arial Narrow', Arial, sans-serif; ">
        <div
            style="height: 100%; padding: 2em; background-image: url(); background-size: 100%;">
            <div style="border-radius: 2em;text-align: -webkit-center;background-color: #9abf15;z-index: 1;position: relative;">
                <img src="https://res.cloudinary.com/techstore/image/upload/v1618082875/edobvt8ghwyblnagtkoj.png" style="width:100px"   />
                <h1 style="color: #000000;margin: .2em; font-size: 2em; font-weight: 100;">Unknow Coder</h1>
                <p style="color: #ffffff;font-weight: 800;font-size:3rem;background-color: #507b00;margin-bottom: 0;padding:1em;">
                    Bienvenido a Unknow Code 
                </p>
                <p style="color: #ffffff;font-weight: 800;font-size:3rem;background-color: #507b00;margin-bottom: 0;padding:1em;">
                    Gracias por registrarte a nuestro E-commerce, espero que los productos sean de su agrado 
                </p>
                
                <p> Thanks for choosing us!</p>
                <p> Unknow Code Team </p>
            </div>
        </div>

    </body>
</html>`


module.exports = {
    registerTemplate
}